...
if ($_SESSION['role'] != 4) {
    ...
    exit();}
$id_questions = intval($_GET['id']);
include "../structura/sql.php";
$result = $con->query("SELECT * FROM savollar WHERE dars_id=$id_questions")->fetch_all(MYSQLI_ASSOC);
...
<head>
  <meta charset="utf-8">
  <meta http-equivi="X-UA-Compatible" content="IE=edge">
    ...
    <tbody>
      ...
      <?php foreach ($result as $row){ ?>
          <tr>
              ...
              <td><?= $row['t_j']; ?></td>
              <td><?= $row['dars_id'] ;?></td>
            <td><a href="update_questions.php?id=<?= $row['id'] ?>"><span class="glyphicon glyphicon-pencil"></span></a>

            <a href="delete_question.php?act=deletequestion&id=<?= $row['id'] ?>"><span class="glyphicon glyphicon-trash"></span></a>
