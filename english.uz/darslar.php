<?php
session_start();
if ($_SESSION['role'] != 1 and $_SESSION['role']!=4) {
    header("Location: index.php");
    exit();
}
$pdo = new PDO("mysql:host=localhost;dbname=english","root","");
    $darslar = $pdo->query("SELECT * FROM darslar")->fetchAll();
?>
<!doctype html>
...
<div class="container">
    ...
    <div class="row">
      ...
      <form method="get" action="test.php">
          <fieldset class="border padding10">
              <legend class="bg-green fg-white">Choose a subject</legend>
              <label>Subject</label>
              <div class="input-control select">
                  <select name="dars">
                      <?php foreach($darslar as $dars):?>
                      <option value="<?=$dars["id"];?>"><?=$dars["nomi"];?></option>
                      <?php endforeach;?>
                  </select>
              </div>
              <br>
              <input type="submit" value="To start the test" class="btn btn-success">
