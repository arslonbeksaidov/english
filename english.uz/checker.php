...
$javoblar = $_POST["savol"];
$tj = 0;
$umumiy = count($javoblar);
foreach($javoblar as $savol=>$javob){
    $temp = $pdo->query("SELECT t_j FROM savollar WHERE id=$savol")->fetch();
    if($temp["t_j"] == $javob) $tj++;
}
?>
...
<div class="container">
    ...
    <div class="row">
      <!--content1  -->
      <h2 class="text-center">The number of questions <?=$umumiy?></h2>
      <h2 class="text-center">The number of correct answers <?=$tj?></h2>
      <h2 class="text-center">Level of performance<?php printf("%.1f",$tj*100/$umumiy)?>%</h2>
    </div>
